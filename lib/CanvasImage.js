...
class CanvasImage extends CanvasElement {
	constructor(imgObj) {
		...
		if (LoaderUtils.getFileType(CI.src.src) === 'svg') {
			...
			console.log(
				'\t\t\t\tConsider rendering the image into a dummy CanvasDrawer and referencing that as your image source instead'
			)
			console.log('\t\t\t', _squigs)
		}
		...
		if (imgObj.fitTo) {
				...
				function _setupOrientation(checkParam) {
					...
					function _orientToOrigin() {
						...
						if (_args.length) {
							...
							if (
								_params[_sourcePropPos] >= 0 &&
								_params[_sourcePropPos] <= _args[5] - _params['source' + _args[1]]
							) {
								...
								for (let item in _fitOrigin)
									_fitOrigin[item] =
										(_fitOrigin[item] - _params['source' + item.toUpperCase()]) * _scale
							}
						}
					}
					...
					for (let item in _fitOrigin) {
						...
						if (_fitOrigin[item] < 0 || _params[_sourcePropSize] < 0)
							_fitOrigin[item] = _params[_sourcePropSize] = 0
					}
				}
			...
			CI.height = _params.height === undefined ? _imgSourceHeight / _imageScale : _params.width
		}
