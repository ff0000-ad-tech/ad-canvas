...
class CanvasImage extends CanvasElement {
	constructor(imgObj) {
		...
		if (LoaderUtils.getFileType(CI.src.src) === 'svg') {
			...
			console.log('\t\t\t', _squigs);
			console.log("\t\t\t\tWARNING: your image source '" + imgObj.source + "' is an SVG and any CanvasBlendMode applied to it will not render in FireFox");
	...
	warn(prop) {
		...
		if (CI['_source' + _altProp] + CI['_source' + prop] > CI.src[_dim]) {
			console.log("\r\rWARNING: on " + CI.stage.canvas.id + ".elements." + CI.id);
			console.log("\tthe combined source" + _altProp + " and source" + prop + " values exceed the " + _dim + " of the CanvasImage.src and will not render properly in most browsers");
			console.log("\t\tconsider revising your source" + prop, '\r\r');
		}
	}
	...
	draw(trans) {
		...
		try {
			...
			console.log("\r\rERROR: Source for CanvasDrawer: '" + CI.stage.canvas.id + "' element: '" + CI.id + "' failed to load; the image source of " + CI.src + " may not be named or loaded properly.");
			console.log("ERROR: CanvasImage requires defined width, height, sourceW, and sourceH values for SVGs, and that data could be undefined:");
			console.log("\t On", String(Device.os + ' ' + Device.osVersion + ': ' + Device.browser + ' - ' + Device.browserVersion).toUpperCase(), ":");
			console.log("\t width:", CI.width, ", height:", CI.height, ", sourceW:", CI._sourceW, ", sourceH:", CI._sourceH, '\r\r');
		}
	}
}
