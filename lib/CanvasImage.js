...
class CanvasImage extends CanvasElement {
	constructor(imgObj) {
		...
		if (imgObj.source.tagName === 'VIDEO') CI.src = imgObj.source
			...
			CI.src = _isSourceCanvas
				? imgObj.source.tagName === 'CANVAS' ? imgObj.source : imgObj.source.canvas
				: imgObj.source.tagName === 'IMG' ? imgObj.source : ImageManager.get(imgObj.source)
		...
		if (CI['_source' + _altProp] + CI['_source' + prop] > CI.src[_dim]) {
			...
			console.log(
				'\tthe combined source' +
					...
					' values exceed the ' +
					_dim +
					' of the CanvasImage.src and will not render properly in most browsers'
			)
	...
	draw(trans) {
		...
		try {
			...
			console.log(
				"\r\rERROR: Source for CanvasDrawer: '" +
					CI.stage.canvas.id +
					"' element: '" +
					CI.id +
