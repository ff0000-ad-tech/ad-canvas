...
class CanvasDrawer {
	...
	*/
	...
	_init(arg) {
		...
		var _css = {
			width:
				arg.css && arg.css.width
					? arg.css.width
					: arg.target && arg.target !== 'body' ? Styles.getCss(arg.target, 'width') : adParams.adWidth,
			height:
				arg.css && arg.css.height
					? arg.css.height
					: arg.target && arg.target !== 'body' ? Styles.getCss(arg.target, 'height') : adParams.adHeight,
		...
		if (CD.qualityScale !== 1) {
			CD.canvas.style.width = CD.canvas.width / CD.qualityScale + 'px'
			CD.canvas.style.height = CD.canvas.height / CD.qualityScale + 'px'
		}
	...
	*/
	...
	/**
		...
		@example
			adData.elements.myCanvasDrawer.removeAllElements();
	*/
	removeAllElements() {
		let CD = this
