/**
	...
	@desc
		...
		<br><br>
*/
function CanvasElement(obj, type) {
	var CDE = this;
	var _params = obj.params;

	CDE._type = obj.type || type;
	CDE.stage = obj.target;

	CDE.id = obj.id || 'canvasdItem' + CDE.stage.elementsLength;
	CDE.stage.elements[CDE.id] = CDE;
	CDE.stage.elementsLength++;

	CDE.visible = obj.visible === false ? false : true;

	CDE.rotation = _params.rotation || 0;
	CDE.scale = _params.scale === 0 ? 0 : (_params.scale || 1);
	CDE.scaleX = _params.scaleX === 0 ? 0 : (_params.scaleX || 1);
	CDE.scaleY = _params.scaleY === 0 ? 0 : (_params.scaleY || 1);

	CDE.alpha = _params.alpha >= 0 ? _params.alpha : 1;
	CDE.blendMode = obj.blendMode || 'source-over';
	CDE._transformOrigin = '0 0';
	...
	CDE._shadowAngle = _dropShadow.angle || 0;
	CDE.shadowDistance = _dropShadow.distance || 0;
	CDE._shadowColor = _dropShadow.color || (_hasShadow ? '#000000' : null);
	CDE.shadowAlpha = _dropShadow.alpha || CDE._shadowAlpha;
	CDE.shadowBlur = _dropShadow.blur || 0;
}
...
CanvasElement.prototype = {
	...
	set shadowColor(value) {
		...
		// CDE._shadowAlpha = Number(CDE._shadowColor.substring(0, CDE._shadowAlpha.lastIndexOf(')')));
	},
	//
	get shadowAlpha() {
	...
	set transformOrigin(value) {
		...
		if (_xTran.indexOf('%') >= 0) {
			...
			_xTran = +_xTran.replace(/px/g, '') / CSB.width;
		} else {
			_xTran = +_xTran / CSB.width;
		}

		if (_yTran.indexOf('%') >= 0) {
			_yTran = +_yTran.replace(/%/g, '') / 100;
		} else if (_yTran.indexOf('px') >= 0) {
			_yTran = +_yTran.replace(/px/g, '') / CSB.height;
		} else {
			_yTran = +_yTran / CSB.height;
		}
		...
		switch (CSB._type) {
			...
			case 'path':
				...
				_yTran -= 0.5;
			default:
				CSB._offsetX = CSB.width * _xTran;
				CSB._offsetY = CSB.height * _yTran;
				break;
			case 'text':
				// THE OFFSET VALUE FOR text OBJECTS IS BEING SET HERE BECAUSE THE ACTUAL width AND height OF THE TEXT IS YET UNKNOWN
				CSB._transOffsetX = _xTran;
				CSB._transOffsetY = _yTran;

				switch (CSB.textAlign) {
					case 'center':
						CSB._transOffsetX -= 0.5;
						break;
					case 'right':
						CSB._transOffsetX *= -1;
						break;
				}
				CSB._wrapText(false, false)
				break;
		}
	},
	...
	// PRIVATE
	_setShadowOffsets: function(ang, dist) {
		var CDE = this;
		var radians = MathUtils.toRadians(CDE._shadowAngle * -1 + 180);
		CDE._shadowOffsetX = Number((Math.cos(radians) * CDE._shadowDistance).toFixed(8));
		CDE._shadowOffsetY = Number((Math.sin(radians) * CDE._shadowDistance).toFixed(8));
