	...
	@property {object} params
		...
		@desc
			...
			<codeblock>
				...
				new CanvasImage({
					target: adData.elements.myCanvasDrawer,
					source: 'smoke',
					id: 'smoke',
					params: {
						x: 0,
			...
			<codeblock>
				var originalCD = new CanvasDrawer({
					id: 'myCanvasDrawer', 
					target: adData.elements.redAdContainer,
					css: {
						x: 0,
...
function CanvasImage(imgObj) {
	var CI = this;
	var _params = imgObj.params = imgObj.params || {};
	CanvasElement.apply(CI, [imgObj, 'image']);
	var _isSourceCanvas = imgObj.target._getIsSourceCanvas(imgObj.source);

	if (imgObj.source.tagName === 'VIDEO') CI.src = imgObj.source;
	else CI.src = _isSourceCanvas ? (imgObj.source.tagName === 'CANVAS' ? imgObj.source : imgObj.source.canvas) : (imgObj.source.tagName === 'IMG' ? imgObj.source : ImageManager.get(imgObj.source));

	if (LoaderUtils.getFileType(CI.src.src) === 'svg') {
		var _squigs = Array(100).join('~');
		trace('\t\t\t', _squigs);
		trace("\t\t\t\tWARNING: your image source '" + imgObj.source + "' is an SVG and any CanvasBlendMode applied to it will not render in FireFox");
		trace("\t\t\t\tConsider rendering the image into a dummy CanvasDrawer and referencing that as your image source instead");
		trace('\t\t\t', _squigs);
	}

	var _imgSourceWidth = CI.src.width || CI.src.getAttribute('width');
	var _imgSourceHeight = CI.src.height || CI.src.getAttribute('height');
	...
	if (imgObj.fitTo) {
		var _fitSpace = imgObj.fitTo;

		// is the image wider than it is tall
		var _widerSource = _imgSourceWidth > _imgSourceHeight;
		// is the target dimension - the _fitSpace - wider than it is tall
		var _widerSpace = _fitSpace.width > _fitSpace.height;
		...
		var _paramsW = ['H', 'W', 'width', 'height', _imgSourceHeight, _imgSourceWidth];
		// array for proportional height, fitting the image to _fitSpace.width
		var _paramsH = ['W', 'H', 'height', 'width', _imgSourceWidth, _imgSourceHeight];
		...
		if (_fitSpace.origin) {
			...
			var _fitSquare = _fitSpace.width === _fitSpace.height;
			...
			function _setupOrientation(checkParam) {
				...
				function _orientToOrigin() {
					...
					if (_args.length) {
						...
						var _sourcePropPos = 'source' + _args[0].toUpperCase();
						...
						if (_params[_sourcePropPos] >= 0 && _params[_sourcePropPos] <= _args[5] - _params['source' + _args[1]]) {
							...
							for (var item in _fitOrigin) _fitOrigin[item] = (_fitOrigin[item] - _params['source' + item.toUpperCase()]) * _scale;
						}
					}
				}

				// this keeps every value of _fitOrigin, as well as _sourceX/Y, within the bounds of the image
				// arguments via _paramsW = ['x', 'W', 'width', 'height', _imgSourceHeight, _imgSourceWidth]
				// arguments via _paramsH = ['y', 'H', 'height', 'width', _imgSourceWidth, _imgSourceHeight];
				function _modifyOutOfBounds() {
					var _subArgs = arguments;
					var _arg1 = _subArgs[0];
					var _arg2 = _subArgs[2];
			...
			if (_fitSpace.debug) {
				...
				// the tall line
				var _tall = new CanvasRect(_args);
				// the wide line
				var _wide = new CanvasRect(_args);

				// circle surrounding the origin point
				_fitOrigin.radius = 10;
				var _circ = new CanvasCircle({
					target: CI.stage,
					params: _fitOrigin,
					stroke: {
						fill: 'yellow',
		...
		var _isMissingW = _params.width === undefined;
		var _isMissingH = _params.height === undefined;
		...
		CI.width = _params.width || _imgSourceWidth;
		CI.height = _params.height || _imgSourceHeight;
	}
	...
	CI.sourceW = _params.sourceW || _imgSourceWidth || CI.width * CI.stage._imageScale;
	CI.sourceH = _params.sourceH || _imgSourceHeight || CI.height * CI.stage._imageScale;
...
CanvasImage.prototype = Object.create(CanvasElement.prototype);
CanvasImage.prototype.constructor = CanvasImage;

CanvasImage.prototype.warn = function(prop) {
	var CI = this;
	var _altProp;
	var _dim;
	switch (prop) {
		...
		case 'Y':
			_altProp = prop === 'X' ? 'W' : 'H';
			_dim = prop === 'X' ? 'width' : 'height';
			break;
		case 'W':
		case 'H':
			_altProp = prop === 'W' ? 'X' : 'Y';
			_dim = prop === 'W' ? 'width' : 'height';
			break;
	}

	if (CI['_source' + _altProp] + CI['_source' + prop] > CI.src[_dim]) {
		trace("\r\rWARNING: on " + CI.stage.canvas.id + ".elements." + CI.id);
		trace("\tthe combined source" + _altProp + " and source" + prop + " values exceed the " + _dim + " of the CanvasImage.src and will not render properly in most browsers");
		trace("\t\tconsider revising your source" + prop, '\r\r');
	}
}

CanvasImage.prototype.draw = function(trans) {
	var CI = this;
	try {
		var _x, _y;
		if (trans) {
		...
		CI.stage.ctx.drawImage(CI.src, CI._sourceX, CI._sourceY, CI._sourceW, CI._sourceH, _x * CI.stage.qualityScale, _y * CI.stage.qualityScale, CI.width * CI.stage.qualityScale, CI.height * CI.stage.qualityScale);
	} catch (err) {
		trace("\r\rERROR: Source for CanvasDrawer: '" + CI.stage.canvas.id + "' element: '" + CI.id + "' failed to load; the image source of " + CI.src + " may not be named or loaded properly.");
		trace("ERROR: CanvasImage requires defined width, height, sourceW, and sourceH values for SVGs, and that data could be undefined:");
		trace("\t On", String(Device.os + ' ' + Device.osVersion + ': ' + Device.browser + ' - ' + Device.browserVersion).toUpperCase(), ":");
		trace("\t width:", CI.width, ", height:", CI.height, ", sourceW:", CI._sourceW, ", sourceH:", CI._sourceH, '\r\r');
	}
}
