...
*/
import CanvasShape from './CavnasShape'

function CanvasPath(pathObj) {
	pathObj.type = 'path';
	CanvasShape.apply(this, arguments);
}

CanvasPath.prototype = Object.create(CanvasShape.prototype);
...
/* ------------------------------------------------
		...
		*/
CanvasPath.prototype._setLineDrawArgs = function(index) {
	var CP = this;
	if (index) {
		CP.drawArgs[index] = {
...
CanvasPath.prototype._setShapeParams = function(args, pathObj) {
	var CP = this;
	CP.width = 0;
	CP.height = 0;
...
CanvasPath.prototype._prepDrawing = function(translation) {
	var CP = this;
	var i, p;
	for (i = 0; i < CP.args.length; i++) {
		for (p = 0; p < CP.args[i].points.length; p += 2) {
			if (translation) {
				CP.drawArgs[i].points[p] = (CP.args[i].points[p] - CP._offsetX) * CP.stage.qualityScale;
				CP.drawArgs[i].points[p + 1] = (CP.args[i].points[p + 1] - CP._offsetY) * CP.stage.qualityScale;
			} else {
				CP.drawArgs[i].points[p] = (CP.args[i].points[p] + CP.x) * CP.stage.qualityScale;
				CP.drawArgs[i].points[p + 1] = (CP.args[i].points[p + 1] + CP.y) * CP.stage.qualityScale;
			}
		}
	}
