...
*/
var CanvasTexture = new function() {
	var CT = this;
	...
	/**
		...
		@desc
			...
			<br><br>
*/
	CT.makeLinearGradient = function(args) {
		var grd = args.target.ctx.createLinearGradient(args.xStart * args.target.qualityScale, args.yStart * args.target.qualityScale, args.xEnd * args.target.qualityScale, args.yEnd * args.target.qualityScale);
	...
	/**
		...
		@desc
			...
			<br><br>
	*/
	CT.makeRadialGradient = function(args) {
		var grd = args.target.ctx.createRadialGradient(args.xInner * args.target.qualityScale, args.yInner * args.target.qualityScale, args.radiusInner * args.target.qualityScale, args.xOuter * args.target.qualityScale, args.yOuter * args.target.qualityScale, args.radiusOuter * args.target.qualityScale);
		for (var i = 0; i < args.colors.length; i++) grd.addColorStop(args.colors[i].stopVal, args.colors[i].color)
		return grd;
	};
	...
	/**
		...
		@desc
			...
			<codeblock>
				adData.elements.myCanvasDrawerPattern = new CanvasDrawer({
					id: 'myCanvasDrawer', 
					target: adData.elements.redAdContainer,
					css: {
						x: 0,
				...
				var pattern = CanvasTexture.makePattern({
					target: adData.elements.myCanvasDrawerPattern,
					source: 'tileJPG', 
		...
		<codeblock>
			adData.elements.myCanvasDrawerCanvasTiling = new CanvasDrawer({
				id: 'myCanvasDrawer', 
				target: adData.elements.redAdContainer,
				css: {
					x: 0,
			...
			var pattern = CanvasTexture.makePattern({
				target: adData.elements.myCanvasDrawer,
				source: adData.elements.myCanvasDrawerPattern.canvas,
