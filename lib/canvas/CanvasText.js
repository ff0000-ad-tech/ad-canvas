...
function CanvasText(textObj) {
	...
	CT.fontSize = _params.fontSize >= 0 ? Number(_params.fontSize.toString().replace(/px/g, '').replace(/pt/g, '')) : 12;
...
CanvasText.prototype.draw = function(translation) {
	var CT = this;
...
/* ------------------------------------------------
	...
	*/
CanvasText.prototype._wrapText = function(translation, draw) {
	var CT = this;
	...
	if (_recalc) {
		...
		CT.deltaProps.strokeOffset = (CT.strokePosition === 'outer' || CT.strokePosition === 'center' ? (CT.strokeWidth / 4) : 0) * CT.stage.qualityScale;
...
/* ------------------------------------------------
		...
		*/
CanvasText.prototype._drawText = function(copy, x, y) {
	var CT = this;
	var _outer = CT.strokePosition === 'outer';
	var _strokeDepth = CT.strokeWidth > 0;
	if (CT.strokeDashSize > 0 && CT.strokeDashGap > 0) CT.stage.ctx.setLineDash([CT.strokeDashSize * CT.stage.qualityScale, CT.strokeDashGap * CT.stage.qualityScale]);
